...
$input = !is_null($raw) ? json_decode($raw, true) : null;
...
if ($success && count($rows) === 1) {
  ...
  if ("pause / play" === $input['action']) {
    $song = SpotifyController::togglePlayback($json['access_token']);
  } else if ("next" === $input['action']) {
    $song = SpotifyController::nextSong($json['access_token']);
  } else if ("previous" === $input['action']) {
    $song = SpotifyController::previousSong($json['access_token']);
  }
} else {
  if (isset($_SERVER['HTTP_AUTHORIZATION'])) {
    ...
    if (strcasecmp($type, "Bearer") === 0) {
      if ("pause / play" === $input['action']) {
        $song = SpotifyController::togglePlayback($data);
      } else if ("next" === $input['action']) {
        $song = SpotifyController::nextSong($data);
      } else if ("previous" === $input['action']) {
        $song = SpotifyController::previousSong($data);
      }
    }
  }
}
