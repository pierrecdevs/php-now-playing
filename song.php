...
if ($success && count($rows) === 1) {
  ...
  $song = SpotifyController::getCurrentlyPlayingSong($user['token']);
  ...
  if (!is_null($json['access_token']))
  {
    ...
    $song = SpotifyController::getCurrentlyPlayingSong($json['access_token']);
  }   
} else {
  if (isset($_SERVER['HTTP_AUTHORIZATION'])) {
    ...
    if (strcasecmp($type, "Bearer") === 0)
      $song = SpotifyController::getCurrentlyPlayingSong($data);
  }
}
